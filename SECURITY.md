...
### 4.5 Import (dump restore) security model

`POST /api/instances/{token}/import` accepts a user-uploaded dump file
(`.sql` for SQL engines, `.js` for MongoDB) and restores it into the
user's own instance. Uploaded dumps are untrusted user content; the
following controls bound the blast radius:

**Execution scope — dump runs inside the user's own container only.**
`MayFly.Api` receives the multipart file, reads the raw bytes (capped at
16 MiB), base64-encodes them, and forwards them to `MayFly.Provisioner`
via the `X-Provisioner-Key`-guarded exec-dump channel
(`POST /internal/instances/{id}/exec-dump`). The API never parses,
interprets, or executes the dump content. The Provisioner base64-decodes
the bytes and pipes them via the exec stdin channel into
`cat > /tmp/dump.<ext>` inside the user's container (binary-safe; no
shell interpolation of dump content). It then invokes the engine's
native client as the DB admin:

| Engine | Client | Invocation |
|---|---|---|
| PostgreSQL | `psql` | `psql -h 127.0.0.1 -U <admin> -d <db> -v ON_ERROR_STOP=1 -f /tmp/dump.sql` as `mayflyadmin` |
| MySQL | `mysql` | `mysql -h 127.0.0.1 -u <admin> <db> < /tmp/dump.sql` as `root` |
| MariaDB | `mariadb` | `mariadb -h 127.0.0.1 -u <admin> <db> < /tmp/dump.sql` as `root` |
| SQL Server | `sqlcmd` | `sqlcmd -C -S 127.0.0.1 -U <admin> -d <db> -b -i /tmp/dump.sql` as `sa` |
| MongoDB | `mongosh` | `mongosh --quiet --host 127.0.0.1 -u <admin> … <db> --file /tmp/dump.js` as `mayflyadmin` |

The dump runs with full admin privileges because initialisation tasks
(e.g. `CREATE TABLE`, `CREATE INDEX`, restoring ownership) require
elevated rights. The user-facing `appuser` credential is never used
during import.

**Confinement is container-level, not admin-level.** Running as admin
inside the container does not expand the attack surface beyond the
container boundary: the container is already egress-blocked
(`mayfly-users` internal network, §3.1), capability-dropped with
`no-new-privileges` (§4.1), resource-limited (memory, CPU, pids),
and TTL-bounded (destroyed when the instance expires, §7). For all
engines except SQL Server, the rootfs is read-only; admin SQL cannot
write arbitrary host paths. The dump content is therefore confined to
the data directory of that user's own disposable container.

**No shell interpolation of dump content.** The dump bytes are piped
over the docker exec stdin channel into `cat > /tmp/dump.<ext>` — the
bytes are never shell-interpolated, never appear in argv, and never touch
an environment variable. The filename `/tmp/dump.<ext>` is fixed (not
derived from the upload's original filename). The identifier fields that
do appear in exec command strings (`adminUser`, `appUser`, `db`) are
validated against a strict `^[A-Za-z0-9_]+$` regex before the exec is
constructed. Admin passwords travel as exec-env vars (e.g. `PGPASSWORD`,
`SQLCMDPASSWORD`), not in argv.

**Base64 API→Provisioner transport.** The dump bytes are carried
base64-encoded in the `DumpContent` JSON field of the internal HTTP
request. Base64 is pure ASCII, so non-ASCII dump content (CJK, emoji,
etc.) does not incur JSON `\uXXXX` escape bloat that could push the
request past Kestrel's body limit. The bytes are decoded back to their
original binary form in the Provisioner before being written to the exec
stdin pipe, guaranteeing an exact byte-for-byte round-trip with no
encoding corruption.

**Size cap (16 MiB → HTTP 413).** `MayFly.Api` reads the raw upload
bytes and enforces a 16 MiB cap on the raw byte count before encoding.
Uploads beyond this are rejected with HTTP 413 before any bytes reach
the Provisioner.

**Container-side timeout and output cap.** The Provisioner sets a
per-exec timeout (bounded by the exec request, max 120 s enforced
server-side). Stdout and stderr are captured into `CappedStream`
buffers (256 KiB); bytes beyond the cap are silently discarded and
`truncated:true` is set in the response. A runaway import that
generates enormous output cannot exhaust Provisioner memory.

**Per-IP rate limit.** Import requests are subject to the global
`perip` rate-limit policy (60 req/min, §6.2). A separate
`import`-scoped policy (6 req/min) further bounds the rate at which
dump bytes can be submitted, reducing the cost of repeated large
uploads.

**Restored data counts against the volume quota.** Data written by an
import is stored on the instance's data volume. The `QuotaEnforcer`
lifecycle tick (§5.2) measures total database size and enforces the
`StorageMb` cap regardless of whether data arrived via seeding, user
queries, or an import. XFS project-quota (§5.1), when enabled, caps
writes at the kernel level.

**PostgreSQL appuser grant after restore.** Because the SQL dump runs
as `mayflyadmin`, any newly created objects are owned by the admin
role and are not initially accessible to `appuser`. After a successful
Postgres restore, a second inline `psql -c` invocation (chained with
`&&` in the same exec) runs:
```sql
GRANT ALL ON ALL TABLES IN SCHEMA public TO <appuser>;
GRANT ALL ON ALL SEQUENCES IN SCHEMA public TO <appuser>;
```
This grants access within `SCHEMA public` only, making the restored
objects immediately available to the user through their `appuser`
connection string without exposing admin credentials.
...
| `import` | `POST /api/instances/{token}/import` | 6 req / min |
