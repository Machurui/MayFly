...
`MayFly.Api` receives the multipart file, validates ownership via
`GetByTokenAsync`, and forwards the raw bytes to `MayFly.Provisioner`
via the `X-Provisioner-Key`-guarded exec-dump channel
(`POST /internal/instances/{id}/exec-dump`). The API never parses,
interprets, or executes the dump content. The Provisioner writes the
bytes to a uniquely-named `/tmp/<guid>.<ext>` path inside the user's
container (via `docker exec tar` pipe — binary-safe, no shell
interpolation of dump content), then invokes the engine's native
client as the DB admin:

| Engine | Client | Invocation |
|---|---|---|
| PostgreSQL | `psql` | `psql -v ON_ERROR_STOP=1 -f /tmp/<file>` as `mayflyadmin` |
| MySQL | `mysql` | `mysql --batch -e "source /tmp/<file>"` as `root` |
| MariaDB | `mariadb` | `mariadb --batch -e "source /tmp/<file>"` as `root` |
| SQL Server | `sqlcmd` | `sqlcmd -b -i /tmp/<file>` (Windows auth, sa-equivalent) |
| MongoDB | `mongosh` | `mongosh --file /tmp/<file>` as `mayflyadmin` |
...
**No shell interpolation of dump content.** The dump bytes are
delivered to the container via a `tar` stdin pipe (not a shell `echo`
or heredoc). The file path embedded in the exec command is a
`/tmp/<guid>` string generated by the Provisioner — not derived from
the upload's original filename. The identifier fields that do appear
in exec command strings (`user`, `authDb`, container name) are
validated against a strict `^[A-Za-z0-9_]+$` regex before the exec
is constructed.

**Size cap (16 MiB → HTTP 413).** `MayFly.Api` enforces a 16 MiB
body limit on the import endpoint; uploads beyond this are rejected
before the bytes reach the Provisioner. This limits the cost of a
single import call and prevents trivially large memory allocations in
the Provisioner's pipe.
...
role and are not initially accessible to `appuser`. After a
successful Postgres restore, the Provisioner calls
`GrantAppuserAccessAsync`, which runs:
```sql
GRANT USAGE ON SCHEMA public TO appuser;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO appuser;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO appuser;
```
This makes the restored objects immediately available to the user
through their `appuser` connection string without exposing admin
credentials.
